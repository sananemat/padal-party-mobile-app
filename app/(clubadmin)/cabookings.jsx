...
// 🔹 Sample matches data (without status)


export default function CABookings() {
  ...
  const [matches, setMatches] = useState([
    {
      ...
      startDateTime: createDate("2024-06-15", "10:00 AM"),
      endDateTime: createDate("2024-06-15", "11:00 AM"),
    },
    {
      id: "2",
      courtId: "2",
      name: "Singles Practice",
      startDateTime: createDate("2025-12-15", "2:00 PM"),
      endDateTime: createDate("2025-12-15", "3:00 PM"),
    },
    {
      id: "3",
      courtId: "1",
      name: "Evening Match",
      startDateTime: createDate("2025-10-05", "6:00 PM"),
      endDateTime: createDate("2025-10-05", "7:00 PM"),
    },
    {
      id: "4",
      courtId: "1",
      name: "Morning Session",
      startDateTime: createDate("2024-06-16", "8:00 AM"),
      endDateTime: createDate("2024-06-16", "9:00 AM"),
    },
    {
      id: "5",
      courtId: "1",
      name: "Night Match",
      startDateTime: createDate("2025-10-05", "8:00 PM"),
      endDateTime: createDate("2025-10-05", "9:00 PM"),
    },
  ]);
  ...
  // 🔹 Categorize matches
  const categorizedMatches = matches.reduce((acc, match) => {
    const matchDate = new Date(Date.UTC(
      match.startDateTime.getUTCFullYear(),
      match.startDateTime.getUTCMonth(),
      match.startDateTime.getUTCDate()
    ));
    ...
    if (matchDate > today) {
      ...
      // Same day - could be Current if time overlaps with now
      acc.Current.push(match); // For simplicity, treat same-day as Upcoming
    }
    return acc;
  }, { Upcoming: [], Current: [], Past: [] });

  const filteredMatches = categorizedMatches[activeTab] || [];

  // 🔹 Find court by ID
  const getCourtById = (id) => SAMPLE_COURTS.find(court => court.id === id);
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
          ...
          filteredMatches.map((match) => {
            ...
            return (
              <CAProfileCard key={match.id} style={styles.matchCard}>
                {/* --- Main Content Row: Image, Info, Buttons --- */}
                <View style={styles.mainContentRow}>
                  ...
                  {court?.image ? (
                    ...
                    <View style={[styles.courtImage, styles.placeholderImage]} />
                  )}

                  {/* --- Info Column (Time, Name, Court) --- */}
                  <View style={styles.infoColumn}>
                    <View style={styles.topRow}>
                      ...
                      <Text style={styles.matchDate}>
                        {match.startDateTime.toLocaleDateString([], {
                          month: 'short',
                          day: 'numeric'
                        })}
                      </Text>
                    ...
                    <Text style={styles.courtName}>{court ? court.name : "Court"}</Text>
                  </View>
                </View>
        ...
        {/* 🔹 Action Cards (only show if matches exist) */}
        {filteredMatches.length > 0 && (
        <>
            <Spacer height={16} />
            <View style={styles.actionCardsRow}>
            <CAProfileCard style={styles.actionCard}>
                <TouchableOpacity style={styles.actionCardContent} onPress={() => setIsTimerModalVisible(true)}>
                <Ionicons name="stopwatch" size={24} color="#eee" />
                <Text style={styles.actionCardText}>Open Match Timer</Text>
                </TouchableOpacity>
            </CAProfileCard>
            <CAProfileCard style={styles.actionCard}>
                <TouchableOpacity style={styles.actionCardContent} onPress={() => router.push("/cacourtavailability")}>
                <Ionicons name="calendar" size={24} color="#eee" />
                <Text style={styles.actionCardText}>Court Availability</Text>
                </TouchableOpacity>
            </CAProfileCard>
            </View>
        </>
...
const styles = StyleSheet.create({
  ...
  matchCard: {
    ...
    marginBottom: 16,
    borderRadius: 12,
    // Ensure no conflicting background if CAProfileCard handles it
  },
  // --- New/Modified Layout Styles ---
  mainContentRow: {
    flexDirection: 'row', // Image, Info Column side-by-side
  ...
  topRow: {
    flexDirection: 'row', // Time and Date side-by-side
    justifyContent: 'space-between', // Push Date to the right
    alignItems: 'flex-start', // Align tops if one wraps
    marginBottom: 4, // Space below time/date row
  },
  actionButtonsRow: {
    flexDirection: 'row',
    justifyContent: 'flex-end', // Align buttons to the right
  },
  // --- End New/Modified Layout Styles ---
  courtImage: {
    width: 60,
    height: 60,
    borderRadius: 8,
  ...
  matchDate: {
    fontSize: 14,
    color: "#6B7280", // Gray date
    textAlign: 'right', // Align right within its flex box if needed
    // minWidth: 50, // Optional: give date a min width
  },
  matchName: {
    fontSize: 16,
    color: "#fff",
